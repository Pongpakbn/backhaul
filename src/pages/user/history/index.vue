    ...
    <v-card>
      ...
      <v-card-text>
        <v-card class="pa-4 d-flex align-center justify-center" flat style="background-color: rgba(242, 169, 0, 0.1)">
          ...
          <p>
            กรุณาเลือกเหตุผลในการยกเลิกการจอง เมื่อการยกเลิกสำเร็จคุณจะได้รับเงินคืนภายใน 7 - 14 วัน
            การยกเลิกจะมีค่าธรรมเนียม โดยจะหัก 3% จากราคาค่าขนส่ง
          </p>
        </v-card>
        <v-radio-group v-model="selectradio" class="mt-2" color="primary" hide-details>
    ...
    <v-card>
      ...
      <v-card-text>
        <v-timeline align="start" density="compact" side="end" truncate-line="both">
          <v-timeline-item
            v-for="(locations, index) in selectedBooking?.locationss || []"
            :key="index"
            :color="getTimelineColor(locations.type)"
            size="small"
            width="100%"
          >
            <template #icon>
              <v-avatar :color="getTimelineColor(locations.type)" size="27" />
            </template>
            <div>
              <div class="font-weight-medium">{{ locations.label }}</div>
              <div class="text-subtitle text-grey-darken-1">{{ locations.address }}</div>
              <div style="color: #005eb8">{{ locations.date }}</div>
            </div>
          </v-timeline-item>
        </v-timeline>
      </v-card-text>
  ...
  export default {
    data () {
      return {
        ...
        dialog_cancle: false,
        activeTab: 0,
        selectedBooking: null, // ✅ เพิ่มไว้เพื่อใช้ดูรายละเอียด
        allBookings: [
          {
            id: '#BH4334513',
            from: 'สงขลา',
            ...
            locationss: [
              ...
              },
              {
                ...
                date: 'ส่งสินค้า : 5 มกราคม 2568 13:00 น. - 15:00 น.',
              },
            ],
          },
          {
